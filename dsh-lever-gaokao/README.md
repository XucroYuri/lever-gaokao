...
**英文名 Latent Edge**：与"潜机"一一对应；技术标识保留 `lever-gaokao` / `dsh-lever-gaokao`（供开发与插件生态）。

**域名策略**：qianji.com/.cn/.app 已被注册（两字拼音域名行业性枯竭），采用变体域名：`qianji.ai` / `getqianji.com` / `latentedge.dev` 等。
