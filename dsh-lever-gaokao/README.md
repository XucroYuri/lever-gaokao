...
> | 名称 | 类型 | 寓意 | 商标/域名状态 |
> |---|---|---|---|
> | **潜机**（qián jī） | 中文主名 | 潜藏的机会；谐音"先机"，易经"潜龙勿用"+禅机思辨 | 商标：公开检索未见 9/41 类占用 ✅；域名：qianji.com/.cn/.app 已占用 ⚠️（可用 qianji.ai 等变体） |
> | **Latent Edge** | 英文名 | latent（潜藏/潜能）+ edge（先机），与中文一一对应 | Hidden Edge / Qianji 已查证占用；latentedge.com/.app 已占（可用 latentedge.dev） |
> | **见潜机** | 备用 1（3 字） | 看见潜藏的机会，先见哲学 | **jianqianji.cn 可注册** ✅ |
> | **机先**（jī xiān） | 备用 2（2 字） | 洞察机先，占得先机（先机倒装） | jixian.com/.cn 已占 |
> | **转机**（zhuǎn jī） | 备用 3（2 字） | 人生的转机，辩证哲学（否极泰来） | 常用词，注册风险高 |
> | **曜机**（yào jī） | 备用 4（2 字） | 光芒照亮的机会（曜=日月光辉），原创组合 | yaoji.com/.cn 已占 |
> | **探机**（tàn jī） | 备用 5（2 字） | 主动探索被忽略的机会，现代直白 | tanji.com/.cn 已占 |
>
> 推荐次序：潜机（主）→ 见潜机（变体）→ 机先/曜机（原创备选）→ 转机（寓意强但注册难）。
> 核心发现：**两字拼音域名资源已枯竭**，三字变体（见潜机）反而有域名空间。
